...
function Trainers() {
  ...
  const [excelTrainers, setExcelTrainer] = useState(null);
  const [excelRegistering, setExcelRegistering] = useState(false);
  ...
  const handleExcelFileChange = async (e) => {
    ...
    setExcelTrainer(jsonData);
  };
  const registerFromExcel = async () => {
    setExcelRegistering(true);
    if (!excelTrainers) {
      toast.error("Please insert an excel file", {
        position: "top-right",
        autoClose: 2000,
        hideProgressBar: false,
        closeOnClick: true,
    ...
    try {
      ...
      setExcelTrainer(null);
      setTrainers([...successfulTrainers, ...trainers]);
    } catch (error) {
          ...
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <form className="p-4 md:p-5">
              <div className="flex items-center justify-center w-full">
                ...
                >
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    ...
                    <p
                      className={`${
                        excelTrainers ? "block" : "hidden"
                      } text-lg text-gray-500 dark:text-gray-400`}
                    >
                      <span className="text-white font-bold">
                        {excelTrainers?.length}
                      </span>{" "}
                      trainer(s) to be registered
                    </p>
                  </div>
                  <input
                    id="dropzone-file"
              ...
              >
                ...
                {excelRegistering ? `Registering....Please wait` : "Register"}
              </button>
            </form>
          </div>
        </div>
                    ...
                    {searchTrainers(trainers)?.map((trainer) => {
                      return (
                        ...
                        >
                          ...
                          <td class="p-4 text-base  text-gray-400 whitespace-nowrap ">
                            {trainer.active ? "ACTIVE" : "IN-ACTIVE"}
                          </td>
